// This code is based on the SimplexNoise for C#
// Author: Benjamin Ward
// Originally authored by Heikki Törmälä
// https://github.com/WardBenjamin/SimplexNoise

/* To use this class, do this:
    Noise.Seed = 1234; //This value is used to calculate the map
    float scale = 0.10f; //To determine the density
    float[][] noiseValues = Noise.Calc2D(mapHeight, mapWidth, scale);

    for (int y = 0; y < fieldCount.Height; y++)
            {
                for (int x = 0; x < fieldCount.Width; x++)
                {
                    GroundType gt = GroundType.GRASS;
                    if (noiseValues[x, y] >= 0 && noiseValues[x, y] < 100) { ft = GroundType.GRASS; }
                    if (noiseValues[x, y] >= 100 && noiseValues[x, y] < 150) { ft = GroundType.DIRT; }
                    if (noiseValues[x, y] >= 150 && noiseValues[x, y] < 200) { ft = GroundType.SAND; }
                    if (noiseValues[x, y] >= 200 && noiseValues[x, y] < 255) { ft = GroundType.WATER; }
                }
            }
* */
